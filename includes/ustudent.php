...
function ustudent()
{
	...
	else
	{
		...
		$data_n["html_head"] = '
			...
			function check()
			{
				if($("#batch_2:checked").val() == undefined && $("#batch_1:checked").val() == undefined)
				{
				...
				if($("#batch_2:checked").val() != undefined && ($("#level_id").val() == "none" || $("#center_id").val() == "none"))
				{
	...
	<form id="form3" name="form3" method="post" action="?action=update&id='.$id.'" onsubmit="return check()">
	  ...
	  <tr style="border-top:#000 thin solid">
		<td colspan="3">
		<input type="checkbox" value="2" name="batch[]" id="batch_2" '.$programme_1_sected.'>
		<label for="batch_2">Diniyat Exams 1437 Hijri</label>
		</td>
	  </tr>
	  <tr>
