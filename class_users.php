		...
		*/
		...
		foreach($result as $a)
		{
			if($a["groups_name"] != "User" && db_has_table($a["groups_name"]))
			{
				...
				{
					$sql2 = "SELECT * FROM ".$groups_name." where email = '".$users_name_temp."' AND (password = '".md5($users_password_temp)."' OR password = '".($users_password_temp)."');";
				}
				// debug_r($sql2);
		...
		//add_to_block();
		// rajpootkhlid@gmail.com, contact@smmoin.com, info@smmoin.com, hasnainwasaya@gmail.com
		if($redirect) {
